...
echo "6) 전체 (docker-compose)"

read -p "선택 (1-6): " DEPLOY_TYPE
...
case $DEPLOY_TYPE in
    1)
        ...
        docker run -d -p ${FRONTEND_PORT:-8501}:8501 \
            --env-file .env \
        ...
        echo "✅ Frontend 배포 완료! http://localhost:${FRONTEND_PORT:-8501}"
        ;;
    ...
    4)
        echo "🌐 Nginx 배포 중..."
        docker build -f Dockerfile.nginx -t lecture-rag-nginx .

        read -p "Frontend 인스턴스 IP 입력 (기본값: localhost): " FRONTEND_IP
        if [ -z "$FRONTEND_IP" ]; then
            FRONTEND_IP="localhost"
        fi

        # nginx.conf에서 frontend 서버 주소 업데이트
        sed -i "s/server frontend:8501;/server $FRONTEND_IP:${FRONTEND_PORT:-8501};/" nginx.conf

        docker run -d -p 80:80 \
            --name lecture-rag-nginx \
            --restart unless-stopped \
            lecture-rag-nginx

        echo "✅ Nginx 배포 완료! http://localhost"
        echo "🔗 Frontend 연결: $FRONTEND_IP:${FRONTEND_PORT:-8501}"
        ;;

    5)
        echo "🚀 Frontend + Nginx 배포 중..."
        ...
        docker run -d -p 80:80 \
            --link lecture-rag-frontend:frontend \
            --name lecture-rag-nginx \
            --restart unless-stopped \
            lecture-rag-nginx

        echo "✅ Frontend + Nginx 배포 완료!"
    ...
    6)
        ...
        echo "⚙️ Backend: http://localhost:${BACKEND_PORT:-8000}"
        echo "🗄️ Database: localhost:${DATABASE_PORT:-5432}"
        ;;
